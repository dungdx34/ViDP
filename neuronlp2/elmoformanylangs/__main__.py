...
def test_main():
  ...
  cmd.add_argument("--input", default="data/test.txt", help="the path to the raw text file.")
  cmd.add_argument("--output_format", default='txt', help='the output format. Supported format includes (hdf5, txt).'
                                                           ' Use comma to separate the format identifiers,'
                                                           ' like \'--output_format=hdf5,plain\'')
  cmd.add_argument("--output_prefix", default="data", help='the prefix of the output file. The output file is in the format of '
                                           '<output_prefix>.<output_layer>.<output_format>')
  cmd.add_argument("--output_layer", default="-1",  help='the target layer to output. 0 for the word encoder, 1 for the first LSTM '
                                          'hidden layer, 2 for the second LSTM hidden layer, -1 for an average'
                                          'of 3 layers.')
  cmd.add_argument("--model", default="/media/vtcc/e0298d8e-ee21-4f1a-9c76-5fd6105872db/178", required=False, help="the path to the model.")
