...
from neuronlp2.io.instance import DependencyInstance, NERInstance, DependencyElmoInstance, DependencyBertInstance, DependencyElmoBertInstance
...
from neuronlp2.elmoformanylangs import Embedder
import torch
...
elmo_all = Embedder('/home/hnc/data/ELMO/viet')
# elmo_all = Embedder('/media/dungdx4/sda2_mnt/178/')
class CoNLLXReaderTransform(object):
    def __init__(self, file_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, feature_bert_path, elmo_path, use_elmo=False, use_bert=False, use_elmo_bert=False):
        self.__source_file = open(file_path, 'r')
        self.__word_alphabet = word_alphabet
        self.__char_alphabet = char_alphabet
        self.__pos_alphabet = pos_alphabet
    ...
    def getNext(self, normalize_digits=True, symbolic_root=False, symbolic_end=False, use_elmo=False, use_bert=False, use_elmo_bert=False, use_test=False):
        lines = []
        line = self.__source_file.readline()
        ...
        if use_elmo:
            elmo_ids = self.elmo.sents2elmo([words])
            return DependencyElmoInstance(Sentence(words, word_ids, char_seqs, char_id_seqs), postags, pos_ids, heads,
                                          types,
                                          type_ids, elmo_ids)
        elif use_bert:
            ...
            # elmo_ids = self.elmo.sents2elmo([words])
            elmo_ids = elmo_all.sents2elmo([words])

            bert_ids = self.features_bert[self.count]
            self.count += 1
            return DependencyElmoBertInstance(Sentence(words, word_ids, char_seqs, char_id_seqs), postags, pos_ids,
                                              heads,
                                              types, type_ids, elmo_ids, bert_ids)
        else:
